...
val_name = "mirrorfairu_{}_{}_{}.txt".format(clf_name,dataset_used,attr)
...
for r in range(repeat_time):
    ...
    pred_de_label = clf.predict_classes(dataset_orig_test.features)
    ...
    # kk
    res = []
    tmp_data = dataset_orig_test.convert_to_dataframe()[0]
    for i in range(len(pred_de)):
        if tmp_data.iloc[i][attr] == 1:
            res.append(pred_de_label[i][0])
            continue
