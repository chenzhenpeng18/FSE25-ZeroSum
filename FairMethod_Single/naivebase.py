...
for r in range(repeat_time):
    ...
    # sr_p = len(dataset_orig_train[(dataset_orig_train['Probability'] == 1) & (dataset_orig_train[attr] == 1)])/len(dataset_orig_train[(dataset_orig_train[attr] == 1)])
    # sr_u = len(dataset_orig_train[(dataset_orig_train['Probability'] == 1) & (dataset_orig_train[attr] == 0)])/len(dataset_orig_train[(dataset_orig_train[attr] == 0)])
    # print("sr_p:", sr_p)
    # print("sr_u:", sr_u)
    ...
    y_val_predlabel = clf.predict(X_val)
    ...
    for i in range(len(y_val_pred)):
        if X_val.iloc[i][attr]==0:
            sorted_data.append(y_val_pred[i][1])
    ...
    pred_del = clf.predict(dataset_orig_test.features)

    res = []
    for index, row in dataset_orig_test.convert_to_dataframe()[0].iterrows():
        if row[attr] == 1:
            res.append(pred_del[int(index)])
        else:
            if pred_de[int(index)][1] >= threshold_value:
                res.append(1)
            else:
                res.append(0)
